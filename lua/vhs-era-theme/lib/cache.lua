...
if vim.fn.isdirectory(nested_cache_dir) == 0 then vim.fn.mkdir(nested_cache_dir, "p") end
...
M.clear = function()
  ...
  if config.disable_cache or config.hot_reload then return end
  if vim.fn.filereadable(cache_path) == 1 then assert(os.remove(cache_path)) end
end

M.exists = function()
  local config = require("vhs-era-theme").config
  if config.disable_cache or config.hot_reload then return false end
...
M.read = function()
  local config = require("vhs-era-theme").config
  if config.disable_cache or config.hot_reload then return nil end
